...
Code changes for this fix can be seen in [User.tsx](src/pages/User.tsx)
...
Code changes to fix this can be seen in [Alert.tsx](src/components/layout/Alert.tsx)
...
`https://` so we need to check in [User.tsx](src/pages/User.tsx) if their
website url starts with `http` before constructing the external link.
Code changes can be see in [User.tsx](src/pages/User.tsx#L48)
...
> [RepoItem.tsx](src/components/repos/RepoItem.tsx#L17)
